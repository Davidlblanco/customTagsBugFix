...
export default function useProductTags(algoliaProductContext?: AlgoliaProductContext | undefined) {
  ...
  useEffect(() => {
    ...
    const _set = async () => {
      ...
      if ((!prod?.categoryTree && !algoliaContext?.categoryTree) && (!prod?.categoryId && !algoliaContext?.categoryId)) {
      ...
      const tags = filterTags(data, prod ?? algoliaContext);
      
      setFilteredTags({
        top: tags?.filter((tag : any) => tag?.customTag?.tagPosition === "top" && tag?.tagTemplate === "customTagTemplate"),
        center: tags?.filter((tag: any) => tag?.customTag?.tagPosition === "center" && tag?.tagTemplate === "customTagTemplate"),
        bottom: tags?.filter((tag: any) => tag?.customTag?.tagPosition === "bottom" && tag?.tagTemplate === "customTagTemplate"),
        tagInsignia: tags?.filter((tag: any) => tag?.tagTemplate === "tagInsigniaTemplate")
      });
    };

    _set();
  }, [data, productContext?.product, algoliaContext]);
