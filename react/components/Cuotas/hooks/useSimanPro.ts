...
export default function UseSimanPro() {
    ...
    const isAvailable = !!productContext?.product?.properties?.some(
        (item) =>
            item.name === "Siman Pro" || item.name === "cemento profesional"
    );
    ...
    const isActive = useMemo(
        () => selected?.some((item) => item.quantity >= 1),
        [selected]
    );
