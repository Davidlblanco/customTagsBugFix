...
export function QuickViewContextProvider({ children }: Props) {
    ...
    function categoryHasQuickview(productContext: Partial<ProductContextState>) {
        ...
        const isEnabledCategory = data.quickviews?.find((c) =>
            categories?.some((ct) => ct === c.category.name.toLowerCase() && c.isActive)
        );
    ...
    useEffect(() => {
        async function fetchData() {
            const { data } = await axios.get("/_v1/admin-quickview/quickviews");
            setData({
                quickviews: data,
                isLoading: false,
            });
